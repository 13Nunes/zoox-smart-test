      ...
      <div class="row q-pb-md">
        <div class="col">
          <h3 class="text-h5 q-my-none flex items-center">
            <q-icon name="view_comfy" /> <span>{{ $t('cities.city') }}</span>
          </h3>
        </div>
      ...
      <q-card>
        ...
        <q-table
          ...
          <q-tr slot="body" slot-scope="props" :props="props">
            ...
            <q-td key="state.title" :props="props">
              {{ props.row.state.title }}
            </q-td>
...
export default {
  ...
  methods: {
    ...mapActions('City', ['getCityList', 'deleteCity']),
    loadDatagrid () {
      this.getCityList().then((response) => {
        this.columns = [
          { name: 'id', field: '_id', label: 'ID', align: 'left', sortable: true, style: 'width: 10%' },
          { name: 'title', field: 'title', label: this.$t('cities.city'), align: 'left', sortable: true, style: 'width: 30%' },
          { name: 'state.title', field: 'state.title', label: this.$t('cities.state'), align: 'left', sortable: true, style: 'width: 30%' },
          { name: 'actions', label: this.$t('generic.actions'), align: 'center', sortable: false, style: 'width: 15%' }
        ]
        this.data = this.cityLst
      }).catch((e) => {
        console.error(e)
      })
    },
    editClick (row) {
      this.$store.commit('City/SET_CITY', row)
      this.$router.push('cities/form')
    },
    deleteClick (row) {
      this.openDialog(this.$t('generic.warning'), this.$t('generic.deleteWarning'), 'negative').onOk(() => {
        let payload = row
        this.deleteCity(payload).catch(err => {
